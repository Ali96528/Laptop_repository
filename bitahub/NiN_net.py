...
def NiN_block(in_channels, out_channnels, kernel_size, strides, padding):
    """NiN块: 1个卷积层+2个1x1卷积层,将单个像素作为样本,通道维度视为不同特征"""
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channnels, kernel_size, strides, padding),
        nn.ReLU(),
...
def NiN_net():
    """
        NiN使用窗口形状为 11x11 、 5x5 和 3x3 的卷积层,输出通道数量与AlexNet中的相同。 每个NiN块后有一个最大汇聚层,汇聚窗口形状为 3x3 ,步幅为2。
    """
    return nn.Sequential(NiN_block(1, 96, kernel_size=11, strides=4, padding=0), nn.MaxPool2d(3, stride=2), NiN_block(96, 256, kernel_size=5, strides=1, padding=2), nn.MaxPool2d(3, stride=2), NiN_block(256, 384, kernel_size=3, strides=1, padding=1), nn.MaxPool2d(3, stride=2), nn.Dropout(0.5),
                         # 标签数是10
                         NiN_block(384, 10, kernel_size=3,
                                   strides=1, padding=1),
                         nn.AdaptiveAvgPool2d((1, 1)),
                         # 将四维输出转化为二维输出,形状是(批量大小，10)
                         nn.Flatten()
                         )
