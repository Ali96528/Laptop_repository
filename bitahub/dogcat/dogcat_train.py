...
parser.add_argument('--arch', default='resnet18',
                    help='resnet18 | resnet34 | resnet50 | resnet101 | resnet152')  # resnet的结构选择
parser.add_argument('--model', help='model file path')  # resnet的结构选择
parser.add_argument('--train_dataset', required=True,
                    help='layer of resnet')  # resnet的结构选择
parser.add_argument('--trainval_dataset', required=True,
                    help='layer of resnet')  # resnet的结构选择
...
trainset = DogCat(data_dir=args.train_dataset, train=True,
                  transform=transforms_dogcat)  # 训练数据集
trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=BATCH_SIZE, shuffle=True, num_workers=4)  # 生成一个个batch进行批训练，组成batch的时候顺序打乱取
trainvalset = DogCat(data_dir=args.trainval_dataset,
                     train=True, transform=transforms_dogcat)  # 训练数据集
trainvalloader = torch.utils.data.DataLoader(
    trainvalset, batch_size=100, shuffle=True, num_workers=2)
# 分类标签
classes = ('dog', 'cat')
...
net = create_model(args.arch, pretrained=pretrained,
                   model_path=args.model, num_classes=2)
            ...
            for p in optimizer.param_groups:
                ...
                for k, v in p.items():
                    if k == 'lr':
                        print('%s:%s' % (k, v))
                        writer.add_scalar(
                            'scalar/learning_rate', v, (epoch + 1))
            lrScheduler.step()
            # 记录参数信息
